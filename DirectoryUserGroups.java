...
public class DirectoryUserGroups {
  ...
  public static void makeUserGroups() {
    ...
    // At first we will generate all objects and arrays needed to initialise everything that the groups will
    ...
    Schedule scheduleEmployer = new Schedule(startEmployer, endEmployer, 9, 0, 17, 0, daysEmployer);
    Schedule scheduleManager = new Schedule(startManager, endManager, 8, 0, 20, 0, daysManager);
    Schedule scheduleAdministrator = new Schedule(startAdministrator, endAdministrator, 0,0,24,60, daysAdministrator);

    // Creating of each group, notice how the blank group gets some null objects, thats because in UseGroup if any of those parts are null,
    // when making a request it will automatically return false
    UserGroup employer = new UserGroup("employer", actionsEmployer, spacesEmployer, scheduleEmployer);
    UserGroup manager = new UserGroup("manager", actionsManager, spacesManager, scheduleManager);
    UserGroup administrator = new UserGroup("administrator", actionsAdministrator, spacesAdministrator, scheduleAdministrator);
